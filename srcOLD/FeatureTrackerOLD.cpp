...
FTrackerOLD::FTrackerOLD(int height, int width)
{
...
void FTrackerOLD::matchFrame(cv::Mat inputFrame)
{
	timespec begin, end;
...
void FTrackerOLD::initialFrame(cv::Mat input_image, vector<cv::KeyPoint> detected_keypoints, cv::Mat descriptors)
{
	frame_count = 0;
...
float FTrackerOLD::time_elapsed(timespec start, timespec end)
{
	return (end.tv_sec + end.tv_nsec/1000000000.0) - (start.tv_sec + start.tv_nsec/1000000000.0);
}

float FTrackerOLD::euclid_distance(cv::Point2f pt1, cv::Point2f pt2)
{
